...
def extract_raw_codex_prompt(prompt: str) -> str:
    """Keep only the user's request from Codex IDE-wrapped prompts."""
    if not prompt:
        return ""
    ...
    for marker in markers:
        ...
        if marker_index >= 0:
            return prompt[marker_index + len(marker):].strip()[:1000]
...
def normalize(data: dict, tool: str) -> dict | None:
    ...
    if tool == "claude":
        ...
        base.update({
            "prompt": extract_raw_codex_prompt(
                data.get("prompt", "")
            ),
